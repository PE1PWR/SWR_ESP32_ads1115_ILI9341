...
void setup()
{
     ...
     tft.fillScreen(ILI9341_BLACK); // Maak display schoon.
     ads.begin();
     #include "SWR_color.h"
}
void display_meterA(float scalA)
{
...
void display_valueB (float SWR)   // Teken de meter uit.
    {
...
void loop()
{
         display_meterA(0); // Dit is de start van de opbouw meterA(PWR)
         display_meterB(0); // Dit is de start van de opbouw meterB(SWR)
         int16_t adc0;  // Lees de input spanning uit van de Pfwd aan de A0 asd1115
         int16_t adc1;  // Lees de input spanning uit van de Prev aan de A1 asd1115 
         adc0 = ads.readADC_SingleEnded(0);
         adc1 = ads.readADC_SingleEnded(1);
         Vfwd = (adc0 * 0.1875)/1000; // omzetten naar de juiste spanning(Wfwd)
         Vrev = (adc1 * 0.1875)/1000; // omzetten naar de juiste spanning(Vrev)
         Pfwd = (Vfwd * Vfwd)*10.5; // Vermogens formule PWR
         Prev = (Vrev * Vrev)*10.5; // Vermogen terug REV
         PfwdA = (Vfwd * Vfwd)*10.5 /10 ; // Aanpassing formule voor vermogensaanduiding power meter     
         ...
         if (Pfwd <= 1) SWR=1;
              ...
              if (PfwdA >= 10)
                 {PfwdA = 10;}           // bescherm meter tegen te ver uitslaan. 100W max
                 input_valueA = (PfwdA);   // Neem waardes mee voor de opbouw meterA (power) 
         ...
         tft.print("F");
         tft.setCursor (x-40,y+20) ; // Zet de power uitlezing digitaal in meterA(POWER) 
         if (Vfwd <= 1.0) // Niveau om door te gaan na 10 watt
         
        {tft.print(Pfwd, 1);    // tot 10 watt met cijfer achter de komma.
         tft.print(" ");
         }
         
           else 
            {
            tft.print(Pfwd, 0); // boven 10 watt zonder cijfer achter de komma.
            tft.print(" ");
            } }      
        ...
        {tft.setTextColor(ILI9341_WHITE, ILI9341_BLACK);
         ...
         tft.print("R");
         tft.setCursor (x+30,y+20);  // Zet de power uitlezing digitaal in meterA(POWER) 
         if (Vrev <= 1.0) // Niveau om door te gaan na 10 watt
        { tft.print(Prev, 1);    // tot 10 watt met cijfer achter de komma.
         tft.print(" ");}
         
           else 
            {
            tft.print(Prev, 0); // boven 10 watt zonder cijfer achter de komma.
            tft.print(" ");
            }}     
      
        {
       // int swr = (SWR / 4.2) * 310;// Aanpassing formule juiste SWR aanduiding digitaal in meter.(SWR)
        tft.setCursor (a-14,b+20);
        tft.print(SWR,1);
        if (SWR >= 3) // Als SWR boven 1:3 komt schakelt dan het beveiligings relays in.
           digitalWrite(relay, HIGH);  // Relay aan boven SWR 1:3
             if (SWR<= 3)
             digitalWrite(relay, LOW); // Relay uit onder SWR 1:3 
             tft.print(" ");
             }
         
         if (SWR > 10)          
                 {SWR=10;}              // bescherm meter tegen te ver uitslaan. SWR 10 max
                 input_valueB = (SWR);  // Neem waardes mee voor de opbouw MeterB (SWR)
