// New is ADAfruit is change by eSPI tft driver.
// Everything works now much better. (15-4-2026)

// This software is free to use for your own projects.
// its full C++

#include <TFT_eSPI.h> // New tft driver
#include <SD.h>
...
TFT_eSPI tft = TFT_eSPI(); // New driver
Adafruit_ADS1115 ads;  // AD converter module
...
void setup() {
  ...
  #include "SWR_color.h"         // load display background information
  pinMode(LEDPIN, OUTPUT);       // RED LED protection is active
  pinMode(PushButton, INPUT);    // Restart button if Relay SWR protection is on.
  pinMode(relay, OUTPUT);        // Relay to stop transmitting when SWR is 1:3
  pinMode(LEDDisp, OUTPUT);      // Digitale signal for display
  digitalWrite(LEDDisp, LOW);    // Display on
}
...
void loop() {
 ...
 // display_meterB(0);  // Start building meterB(SWR)
...

  {
    ...
    tft.setCursor(a - 42, b + 20);
    tft.print("SWR = 1:");
    tft.setCursor(a + 5, b + 20);
    tft.print(SWR, 1);
