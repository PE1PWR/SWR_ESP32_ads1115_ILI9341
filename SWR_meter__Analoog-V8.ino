...
void loop()
{
         ...
         if (PfwdA >= 10)
             {PfwdA = 10;}           // Protection analog meter. 100W max
       ...
       // int swr = (SWR / 4.2) * 310;// Adjust formula for correct SWR digital whitin meter.(SWR)
        ...
        if (SWR >= 3) // If SWR above 1:3 powerup protection relais.
           digitalWrite(relay, HIGH);  // Relais on above SWR 1:3
             if (SWR<= 3)
             digitalWrite(relay, LOW); // Relay off below SWR 1:3 
             tft.print(" ");
             }
         
        if (SWR > 10)          
             {SWR=10;}              //  // Protection analog meter. max SWR 1:10 max
